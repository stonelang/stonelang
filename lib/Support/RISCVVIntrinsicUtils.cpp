...
void RVVType::initStoneBuiltinStr() {
  assert(isValid() && "RVVType is invalid");
  assert(isVector() && "Handle Vector type only");

  StoneBuiltinStr = "__rvv_";
  switch (ScalarType) {
  case ScalarTypeKind::Boolean:
    StoneBuiltinStr += "bool" + utostr(64 / *Scale) + "_t";
    return;
  case ScalarTypeKind::Float:
    StoneBuiltinStr += "float";
    break;
  case ScalarTypeKind::BFloat:
    StoneBuiltinStr += "bfloat";
    break;
  case ScalarTypeKind::SignedInteger:
    StoneBuiltinStr += "int";
    break;
  case ScalarTypeKind::UnsignedInteger:
    StoneBuiltinStr += "uint";
    break;
  default:
    llvm_unreachable("ScalarTypeKind is invalid");
  }
  StoneBuiltinStr += utostr(ElementBitwidth) + LMUL.str() +
