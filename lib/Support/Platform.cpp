#include "stone/Support/Platform.h"
...
llvm::StringRef stone::PlatformString(PlatformKind platform) {
  switch (platform) {
...
llvm::StringRef stone::PrettyPlatformString(PlatformKind platform) {
  switch (platform) {
  case PlatformKind::None:
    return "*";
#define AVAILABILITY_PLATFORM(X, PrettyName)                                   \
...
std::optional<PlatformKind> stone::PlatformFromString(llvm::StringRef Name) {
  if (Name == "*")
