#include "stone/Support/FileSystem.h"
...
tryToOpenTemporaryFile(std::optional<llvm::raw_fd_ostream> &openedStream,
  ...
  const unsigned stones = llvm::sys::fs::all_read | llvm::sys::fs::all_write;
  int fd;
  std::error_code EC = llvm::sys::fs::createUniqueFile(
      tempPath, fd, tempPath, llvm::sys::fs::OF_None, stones);
...
std::error_code stone::atomicallyWritingToFile(
    const StringRef outputPath,
    const llvm::function_ref<void(llvm::raw_pwrite_stream &)> action) {
  namespace fs = llvm::sys::fs;

  // FIXME: This is mostly a simplified version of
  // clang::SCInstance::createOutputFile. It would be great to share the
  // implementation.
...
stone::areFilesDifferent(const llvm::Twine &source,
...
std::error_code stone::moveFileIfDifferent(const llvm::Twine &source,
                                           const llvm::Twine &destination) {
  namespace fs = llvm::sys::fs;
