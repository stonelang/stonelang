...
class SrcUnit final {
  unsigned bufferID = 0; ///< Assigned by the source manager.
  llvm::StringRef name;  ///< File name or "-" for stdin.

  //--- Lexing Range Information ---
  unsigned offset = 0;    ///< Where lexing begins (inclusive).
  unsigned endOffset = 0; ///< Artificial EOF offset (where lexing stops).
  unsigned bufferEndOffset =
      0; ///< Physical buffer size (used for bounds check).
...
public:
  ...
  /// Reset internal state for a fresh lex.
  void Clear();
