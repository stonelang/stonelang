...
SrcLoc InFlightDiagnostic::GetLocForEndOfToken(const SrcMgr &sm, SrcLoc loc) {
  return Engine->GetLexing()->GetLocForEndOfToken(sm, loc);
}
...
InFlightDiagnostic::fixItInsertAfter(SrcLoc L, StringRef FormatString,
  ...
  L = Engine->GetLexing()->GetLocForEndOfToken(Engine->SourceMgr, L);
...
//     auto underlyingTy = aliasDecl->getType();
...
CreateDiagnosticInfoForDecl(const Diagnostic &diagnostic) {
  ...
  if (loc.isInvalid()) {
    ...
    assert(decl && "Cannot create diagnostics for a null Decl!");
    // TODO:
