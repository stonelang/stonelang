//===- StoneOpenCLBuiltinEmitter.cpp - Generate Stone OpenCL Builtin handling
//
...
// stone/lib/Sema/OpenCLBuiltins.td and produce builtin handling code for
...
private:
  ...
  // Emit a function returning the stone QualType instance associated with
...
std::string OpenCLBuiltinFileEmitterBase::getTypeString(const Record *Type,
  ...
  auto PrintAddrSpace = [&S](StringRef AddrSpace) {
    S += StringSwitch<const char *>(AddrSpace)
             .Case("stone::LangAS::opencl_private", "__private")
             .Case("stone::LangAS::opencl_global", "__global")
             .Case("stone::LangAS::opencl_constant", "__constant")
             .Case("stone::LangAS::opencl_local", "__local")
             .Case("stone::LangAS::opencl_generic", "__generic")
             .Default("__private");
    S += " ";
  };
  if (Flags.IsPointer) {
...
void stone::EmitStoneOpenCLBuiltins(RecordKeeper &Records, raw_ostream &OS) {
  BuiltinNameEmitter NameChecker(Records, OS);
  NameChecker.Emit();
}

void stone::EmitStoneOpenCLBuiltinHeader(RecordKeeper &Records,
                                         raw_ostream &OS) {
  OpenCLBuiltinHeaderEmitter HeaderFileGenerator(Records, OS);
  HeaderFileGenerator.emit();
}

void stone::EmitStoneOpenCLBuiltinTests(RecordKeeper &Records,
                                        raw_ostream &OS) {
