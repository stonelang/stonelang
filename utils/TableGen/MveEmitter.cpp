//===- MveEmitter.cpp - Generate arm_mve.h for use with stone -*- C++ -*-=====//
//
...
// all inline wrappers on the underlying stone builtins, or to define a single
...
// function to be mapped to a stone builtin even though it doesn't have the
// same name, and then declare all the user-facing MVE function names with that
// attribute, mapping each one directly to the stone builtin. And the
...
// call.
//
// The downside of this is that if every stone builtin has to correspond
...
// sequence of IRBuilder calls that stone will need to make.
//
//===----------------------------------------------------------------------===//
...
// Result subclass that retrieves one of the arguments to the stone builtin
...
class ACLEIntrinsic {
  ...
  // Name of the architecture extension, used in the Stone builtin name
...
// -----------------------------------------------------------------------------
// The class used for generating arm_mve.h and related Stone bits
//
...
void MveEmitter::EmitHeader(raw_ostream &OS) {
  ...
  for (const auto &kv : ScalarTypes) {
    ...
    OS << "typedef __attribute__((__neon_vector_type__(" << VT->lanes()
       << "), __stone_arm_mve_strict_polymorphism)) " << ST->cName() << " "
       << VT->cName() << ";\n";
  ...
  for (const auto &kv : ACLEIntrinsics) {
    ...
    for (bool Polymorphic : {false, true}) {
      ...
      for (bool UserNamespace : {false, true}) {
        ...
        // provided stone recognizes them as builtins, and has the
        ...
        // was known to stone already.
        //
        // The declarations use __attribute__(__stone_arm_builtin_alias),
        ...
        // has exactly the effect we want: first stone resolves the
        ...
        // to a polymorphic function: this way, stone will print just
        ...
        OS << "static __inline__ __attribute__(("
           ...
           << "__stone_arm_builtin_alias(__builtin_arm_mve_" << Int.fullName()
           << ")))\n"
...
// -----------------------------------------------------------------------------
// The class used for generating arm_cde.h and related Stone bits
//
...
void CdeEmitter::EmitHeader(raw_ostream &OS) {
  ...
  for (const auto &kv : ACLEIntrinsics) {
    ...
    for (bool Polymorphic : {false, true}) {
      ...
      OS << "static __inline__ __attribute__(("
         << (Polymorphic ? "__overloadable__, " : "")
         << "__stone_arm_builtin_alias(__builtin_arm_" << Int.builtinExtension()
         << "_" << Int.fullName() << ")))\n"
         << RetTypeName << FunctionName << "(" << ArgTypesString << ");\n";
    }
  }
