//===- StoneSyntaxEmitter.cpp - Generate stone Syntax Tree nodes ----------===//
//
...
// See stone/include/stone/Tooling/Syntax/{Syntax,Nodes}.td
//
// The -gen-stone-syntax-node-list backend produces a .inc with macro calls
//   NODE(Kind, BaseKind)
//   ABSTRACT_NODE(Type, Base, FirstKind, LastKind)
// similar to those for AST nodes such as AST/DeclNodes.inc.
//
// The -gen-stone-syntax-node-classes backend produces definitions for the
...
void stone::EmitStoneSyntaxNodeList(llvm::RecordKeeper &Records,
                                    llvm::raw_ostream &OS) {
...
void stone::EmitStoneSyntaxNodeClasses(llvm::RecordKeeper &Records,
                                       llvm::raw_ostream &OS) {
  llvm::emitSourceFileHeader("Syntax tree node list", OS, Records);
  Hierarchy H(Records);
