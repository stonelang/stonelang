//=- StoneSACheckersEmitter.cpp - Generate Stone SA checkers tables -*- C++ -*-
//
...
//===----------------------------------------------------------------------===//
//
// This tablegen backend emits Stone Static Analyzer checkers tables.
//
//===----------------------------------------------------------------------===//
...
static std::string getCheckerDocs(const Record &R) {
  ...
  return (llvm::Twine("https://stone.llvm.org/docs/analyzer/checkers.html#") +
          CheckerFullName)
      .str();
}
...
void stone::EmitStoneSACheckers(RecordKeeper &Records, raw_ostream &OS) {
