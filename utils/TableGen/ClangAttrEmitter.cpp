//===- StoneAttrEmitter.cpp - Generate Stone attribute handling =-*- C++ -*--=//
//
...
//===----------------------------------------------------------------------===//
//
// These tablegen backends emit Stone attribute processing code
//
//===----------------------------------------------------------------------===//
...
GetFlattenedSpellings(const Record &Attr) {
  ...
  for (const auto &Spelling : Spellings) {
    ...
    } else if (Variety == "Stone") {
      Ret.emplace_back("GNU", std::string(Name), "", false, *Spelling);
      Ret.emplace_back("CXX11", std::string(Name), "stone", false, *Spelling);
      if (Spelling->getValueAsBit("AllowInC"))
        Ret.emplace_back("C23", std::string(Name), "stone", false, *Spelling);
    } else
...
static void emitStoneAttrLateParsedListImpl(RecordKeeper &Records,
                                            raw_ostream &OS,
...
static void emitStoneAttrLateParsedList(RecordKeeper &Records,
                                        raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_LATE_PARSED_LIST)\n";
  emitStoneAttrLateParsedListImpl(Records, OS, LateAttrParseKind::Standard);
  OS << "#endif // STONE_ATTR_LATE_PARSED_LIST\n\n";
}

static void emitStoneAttrLateParsedExperimentalList(RecordKeeper &Records,
                                                    raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_LATE_PARSED_EXPERIMENTAL_EXT_LIST)\n";
  emitStoneAttrLateParsedListImpl(Records, OS,
                                  LateAttrParseKind::ExperimentalExt);
  OS << "#endif // STONE_ATTR_LATE_PARSED_EXPERIMENTAL_EXT_LIST\n\n";
}
...
struct PragmaStoneAttributeSupport {
  ...
  PragmaStoneAttributeSupport(RecordKeeper &Records);
...
static bool isSupportedPragmaStoneAttributeSubject(const Record &Subject) {
  // FIXME: #pragma stone attribute does not currently support statement
  ...
  if (Subject.isSubClassOf("SubsetSubject"))
    return isSupportedPragmaStoneAttributeSubject(
...
PragmaStoneAttributeSupport::PragmaStoneAttributeSupport(
...
static PragmaStoneAttributeSupport &
getPragmaAttributeSupport(RecordKeeper &Records) {
  static PragmaStoneAttributeSupport Instance(Records);
  return Instance;
}

void PragmaStoneAttributeSupport::emitMatchRuleList(raw_ostream &OS) {
...
bool PragmaStoneAttributeSupport::isAttributedSupported(
    const Record &Attribute) {
  // If the attribute explicitly specified whether to support #pragma stone
  ...
  for (const auto *Subject : Subjects) {
    if (!isSupportedPragmaStoneAttributeSubject(*Subject))
      continue;
...
void
PragmaStoneAttributeSupport::generateStrictConformsTo(const Record &Attr,
                                                      raw_ostream &OS) {
  ...
  for (const auto *Subject : Subjects) {
    ...
    assert(It != SubjectsToRules.end() &&
           "This attribute is unsupported by #pragma stone attribute");
...
void PragmaStoneAttributeSupport::generateParsingHelpers(raw_ostream &OS) {
...
static void emitStoneAttrTypeArgList(RecordKeeper &Records, raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_TYPE_ARG_LIST)\n";
  std::vector<Record *> Attrs = Records.getAllDerivedDefinitions("Attr");
  ...
  OS << "#endif // STONE_ATTR_TYPE_ARG_LIST\n\n";
}

/// Emits the parse-arguments-in-unevaluated-context property for
/// attributes.
static void emitStoneAttrArgContextList(RecordKeeper &Records, raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_ARG_CONTEXT_LIST)\n";
  ...
  OS << "#endif // STONE_ATTR_ARG_CONTEXT_LIST\n\n";
}
...
static void emitStoneAttrVariadicIdentifierArgList(RecordKeeper &Records,
                                                   raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_VARIADIC_IDENTIFIER_ARG_LIST)\n";
  std::vector<Record *> Attrs = Records.getAllDerivedDefinitions("Attr");
  ...
  OS << "#endif // STONE_ATTR_VARIADIC_IDENTIFIER_ARG_LIST\n\n";
}
...
static void emitStoneAttrUnevaluatedStringLiteralList(RecordKeeper &Records,
                                                      raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_STRING_LITERAL_ARG_LIST)\n";
  ...
  OS << "#endif // STONE_ATTR_STRING_LITERAL_ARG_LIST\n\n";
}

// Emits the first-argument-is-identifier property for attributes.
static void emitStoneAttrIdentifierArgList(RecordKeeper &Records, raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_IDENTIFIER_ARG_LIST)\n";
  std::vector<Record*> Attrs = Records.getAllDerivedDefinitions("Attr");
  ...
  OS << "#endif // STONE_ATTR_IDENTIFIER_ARG_LIST\n\n";
}

// Emits the indexed-argument-is-identifier property for attributes.
static void emitStoneAttrStrictIdentifierArgAtIndexList(RecordKeeper &Records,
                                                        raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_STRICT_IDENTIFIER_ARG_AT_INDEX_LIST)\n";
  std::vector<Record *> Attrs = Records.getAllDerivedDefinitions("Attr");
  ...
  OS << "#endif // STONE_ATTR_STRICT_IDENTIFIER_ARG_AT_INDEX_LIST\n\n";
}
...
static void emitStoneAttrThisIsaIdentifierArgList(RecordKeeper &Records,
                                                  raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_THIS_ISA_IDENTIFIER_ARG_LIST)\n";
  std::vector<Record *> Attrs = Records.getAllDerivedDefinitions("Attr");
  for (const auto *A : Attrs) {
    // Determine whether the first argument is a variadic identifier.
    std::vector<Record *> Args = A->getValueAsListOfDefs("Args");
  ...
  OS << "#endif // STONE_ATTR_THIS_ISA_IDENTIFIER_ARG_LIST\n\n";
}

static void emitStoneAttrAcceptsExprPack(RecordKeeper &Records,
                                         raw_ostream &OS) {
  OS << "#if defined(STONE_ATTR_ACCEPTS_EXPR_PACK)\n";
  ParsedAttrMap Attrs = getParsedAttrList(Records);
  for (const auto &I : Attrs) {
    const Record &Attr = *I.second;
  ...
  OS << "#endif // STONE_ATTR_ACCEPTS_EXPR_PACK\n\n";
}
...
void stone::EmitStoneAttrClass(RecordKeeper &Records, raw_ostream &OS) {
  emitSourceFileHeader("Attribute classes' definitions", OS, Records);

  OS << "#ifndef LLVM_STONE_ATTR_CLASSES_INC\n";
  OS << "#define LLVM_STONE_ATTR_CLASSES_INC\n\n";

  emitAttributes(Records, OS, true);

  OS << "#endif // LLVM_STONE_ATTR_CLASSES_INC\n";
}

// Emits the class method definitions for attributes.
void stone::EmitStoneAttrImpl(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitStoneAttrList(RecordKeeper &Records, raw_ostream &OS) {
  emitSourceFileHeader("List of all attributes that Stone recognizes", OS,
                       Records);
...
// Emits the enumeration list for attributes.
void EmitStoneAttrSubjectMatchRuleList(RecordKeeper &Records, raw_ostream &OS) {
  emitSourceFileHeader(
      "List of all attribute subject matching rules that Stone recognizes", OS,
      Records);
  PragmaStoneAttributeSupport &PragmaAttributeSupport =
...
void EmitStoneAttrPCHRead(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitStoneAttrPCHWrite(RecordKeeper &Records, raw_ostream &OS) {
...
static void GenerateHasAttrSpellingStringSwitch(
  ...
  for (const auto &[Attr, Spelling] : Attrs) {
    ...
    //
    // Stone-specific attributes have the same kind of version information
    // associated with them. This version is typically the default value (1).
    // These version values are stone-specific and should typically be
    ...
    if (Spelling.nameSpace().empty() || Scope == Spelling.nameSpace()) {
      ...
      // not inferred from Stone/GCC spellings) have a version that's
...
void EmitStoneRegularKeywordAttributeInfo(RecordKeeper &Records,
                                          raw_ostream &OS) {
  emitSourceFileHeader(
...
void EmitStoneAttrHasAttrImpl(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitStoneAttrSpellingListIndex(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitStoneAttrASTVisitor(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitStoneAttrTemplateInstantiateHelper(const std::vector<Record *> &Attrs,
                                            raw_ostream &OS,
                                            bool AppliesToDecl) {
...
void EmitStoneAttrTemplateInstantiate(RecordKeeper &Records, raw_ostream &OS) {
  ...
  EmitStoneAttrTemplateInstantiateHelper(Attrs, OS, /*AppliesToDecl*/false);
  OS << "}\n\n"
     << "Attr *instantiateTemplateAttributeForDecl(const Attr *At,\n"
     << " ASTContext &C, Sema &S,\n"
     << "        const MultiLevelTemplateArgumentList &TemplateArgs) {\n";
  EmitStoneAttrTemplateInstantiateHelper(Attrs, OS, /*AppliesToDecl*/true);
  OS << "}\n\n"
     << "} // end namespace sema\n"
     << "} // end namespace stone\n";
}
...
void EmitStoneAttrParsedAttrList(RecordKeeper &Records, raw_ostream &OS) {
  emitSourceFileHeader("List of all attributes that Stone recognizes", OS,
                       Records);
...
static void
emitAttributeMatchRules(PragmaStoneAttributeSupport &PragmaAttributeSupport,
                        raw_ostream &OS) {
...
void GenerateHandleAttrWithDelayedArgs(RecordKeeper &Records, raw_ostream &OS) {
  ...
  OS << "  }\n";
  OS << "  stone::Attr *CreatedAttr = nullptr;\n";
...
void EmitStoneAttrParsedAttrImpl(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitStoneAttrParsedAttrKinds(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitStoneAttrTextNodeDump(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitStoneAttrNodeTraverse(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitStoneAttrParserStringSwitches(RecordKeeper &Records, raw_ostream &OS) {
  emitSourceFileHeader("Parser-related llvm::StringSwitch cases", OS, Records);
  emitStoneAttrArgContextList(Records, OS);
  emitStoneAttrIdentifierArgList(Records, OS);
  emitStoneAttrUnevaluatedStringLiteralList(Records, OS);
  emitStoneAttrVariadicIdentifierArgList(Records, OS);
  emitStoneAttrThisIsaIdentifierArgList(Records, OS);
  emitStoneAttrAcceptsExprPack(Records, OS);
  emitStoneAttrTypeArgList(Records, OS);
  emitStoneAttrLateParsedList(Records, OS);
  emitStoneAttrLateParsedExperimentalList(Records, OS);
  emitStoneAttrStrictIdentifierArgAtIndexList(Records, OS);
}

void EmitStoneAttrSubjectMatchRulesParserStringSwitches(RecordKeeper &Records,
                                                        raw_ostream &OS) {
  getPragmaAttributeSupport(Records).generateParsingHelpers(OS);
}

void EmitStoneAttrDocTable(RecordKeeper &Records, raw_ostream &OS) {
  emitSourceFileHeader("Stone attribute documentation", OS, Records);
...
static void WriteDocumentation(RecordKeeper &Records,
  ...
  // Note: "#pragma stone attribute" is handled outside the spelling kinds loop
  // so it must be last.
...
void EmitStoneAttrDocs(RecordKeeper &Records, raw_ostream &OS) {
...
void EmitTestPragmaAttributeSupportedAttributes(RecordKeeper &Records,
                                                raw_ostream &OS) {
  PragmaStoneAttributeSupport Support = getPragmaAttributeSupport(Records);
  ParsedAttrMap Attrs = getParsedAttrList(Records);
  OS << "#pragma stone attribute supports the following attributes:\n";
  for (const auto &I : Attrs) {
    ...
    for (const auto &Subject : llvm::enumerate(Subjects)) {
      if (!isSupportedPragmaStoneAttributeSubject(*Subject.value()))
        continue;
      if (PrintComma)
        OS << ", ";
      PrintComma = true;
      PragmaStoneAttributeSupport::RuleOrAggregateRuleSet &RuleSet =
