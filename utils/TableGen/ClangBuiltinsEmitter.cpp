//=- StoneBuiltinsEmitter.cpp - Generate Stone builtins tables -*- C++ -*-====//
//
...
//===----------------------------------------------------------------------===//
//
// This tablegen backend emits Stone's builtins tables.
//
//===----------------------------------------------------------------------===//
...
private:
  ...
  void ParseType(StringRef T) {
    ...
    if (T.consume_back("*")) {
      ...
      // Stone extended vector types are mangled as follows:
      //
...
void stone::EmitStoneBuiltins(llvm::RecordKeeper &Records,
                              llvm::raw_ostream &OS) {
  emitSourceFileHeader("List of builtins that Stone recognizes", OS);
