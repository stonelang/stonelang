//=== StoneTypeNodesEmitter.cpp - Generate type node tables -----*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This tblgen backend emits the node table (the .def file) for Stone
// type nodes.
//
...
void TypeNodeEmitter::emit() {
  ...
  emitSourceFileHeader("An x-macro database of Stone type nodes", Out, Records);
...
void stone::EmitStoneTypeNodes(RecordKeeper &records, raw_ostream &out) {
