//=- StoneDiagnosticsEmitter.cpp - Generate Stone diagnostics tables -*- C++ -*-
//
...
//===----------------------------------------------------------------------===//
//
// These tablegen backends emit Stone diagnostics tables.
//
//===----------------------------------------------------------------------===//
...
static void groupDiagnostics(const std::vector<Record*> &Diags,
  ...
  // groups (these are warnings that GCC supports that stone never produces).
  for (unsigned i = 0, e = DiagGroups.size(); i != e; ++i) {
...
static void verifyDiagnosticWording(const Record &Diag) {
  ...
  // FIXME: This could also be improved by looking for instances of stone or
  // gcc in the diagnostic and recommend Stone or GCC instead. However, this
  // runs into odd situations like [[stone::warn_unused_result]],
  // #pragma stone, or --unwindlib=libgcc.
}

/// StoneDiagsDefsEmitter - The top-level class emits .def files containing
/// declarations of Stone diagnostics.
void stone::EmitStoneDiagsDefs(RecordKeeper &Records, raw_ostream &OS,
...
void stone::EmitStoneDiagGroups(RecordKeeper &Records, raw_ostream &OS) {
...
void stone::EmitStoneDiagsIndexName(RecordKeeper &Records, raw_ostream &OS) {
  const std::vector<Record*> &Diags =
    Records.getAllDerivedDefinitions("Diagnostic");
...
void writeDiagnosticText(DiagnosticTextBuilder &Builder, const Record *R,
  ...
  StringRef Text = R->getValueAsString("Summary");
  if (Text == "%0")
    OS << "The text of this diagnostic is not controlled by Stone.\n\n";
  else {
...
void stone::EmitStoneDiagDocs(RecordKeeper &Records, raw_ostream &OS) {
