//===- RISCVVEmitter.cpp - Generate riscv_vector.h for use with stone -----===//
//
...
//
// See also the documentation in include/stone/Basic/riscv_vector.td.
//
//===----------------------------------------------------------------------===//

#include "stone/Support/RISCVVIntrinsicUtils.h"
...
void RVVEmitter::createHeader(raw_ostream &OS) {
  ...
  OS << "#pragma stone riscv intrinsic vector\n\n";

  printHeaderCode(OS);

  auto printType = [&](auto T) {
    OS << "typedef " << T->getStoneBuiltinStr() << " " << T->getTypeStr()
       << ";\n";
  };
