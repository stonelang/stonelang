#ifndef STONE_CORE_OUTPUTPATHS_H
#define STONE_CORE_OUTPUTPATHS_H
...
struct SupplementaryOutputPaths final {
  ...
  /// It is valid whenever there are any inputs.
  ///
  /// Stone's compilation model means that Make-style dependencies aren't
  ...
  /// The path to which we should output a Stone "reference dependencies" file.
  /// It is valid whenever there are any inputs.
  ///
  /// "Reference dependencies" track dependencies on a more fine-grained level
  /// than just "this file depends on that file". With Stone's "implicit
  /// visibility" within a module, that becomes very important for any sort of
  /// incremental build. These files are consumed by the Stone driver to decide
  ...
  /// The output path of Stone semantic info for this module.
  ...
  /// output
  void ForEachSetOutput(std::function<void(const std::string &)> fn) const {
...
struct FileSpecificPaths final {
  ...
  std::string mainSrcnameForDebugInfo;

  FileSpecificPaths(StringRef outputFilename = llvm::StringRef(),
                    StringRef indexUnitOutputFilename = llvm::StringRef(),
                    StringRef mainSrcnameForDebugInfo = llvm::StringRef(),
                    SupplementaryOutputPaths supplementaryOutputPaths =
                        SupplementaryOutputPaths())
      : outputFilename(outputFilename),
        indexUnitOutputFilename(indexUnitOutputFilename),
        supplementaryOutputPaths(supplementaryOutputPaths),
        mainSrcnameForDebugInfo(mainSrcnameForDebugInfo) {}
