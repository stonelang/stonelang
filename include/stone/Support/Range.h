#ifndef STONE_SUPPORT_RANGE_H
#define STONE_SUPPORT_RANGE_H
...
template <class T, bool IsEnum = std::is_enum<T>::value> struct IntRangeTraits;

template <class T> struct IntRangeTraits<T, /*is enum*/ false> {
...
template <class T> struct IntRangeTraits<T, /*is enum*/ true> {
...
/// like an ArrayRef.
template <class T = unsigned, class Traits = IntRangeTraits<T>> class IntRange {
  T Begin;
  T End;
