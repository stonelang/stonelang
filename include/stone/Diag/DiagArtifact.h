...
/// reflection of artifact kind information. It's used when constructing or
/// formatting diagnostics that take arguments of different types.
template <typename T>
struct DiagArtifactKindOf; // Undefined base — will fail if unsupported type is
                           // used

// === String types ===
template <> struct DiagArtifactKindOf<llvm::StringRef> {
  static constexpr DiagArtifactKind kind = DiagArtifactKind::String;
};

template <> struct DiagArtifactKindOf<std::string> {
  static constexpr DiagArtifactKind kind = DiagArtifactKind::String;
};

// === Integer types ===
template <> struct DiagArtifactKindOf<int> {
  static constexpr DiagArtifactKind kind = DiagArtifactKind::Integer;
};

template <> struct DiagArtifactKindOf<unsigned> {
...
// === AST-specific types ===
template <> struct DiagArtifactKindOf<Decl *> {
  static constexpr DiagArtifactKind kind = DiagArtifactKind::Decl;
};

template <> struct DiagArtifactKindOf<Type *> {
